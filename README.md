...
## 🛡️ Архитектурный принцип: Разделение MCP на Control Plane и Data Plane

Для обеспечения **строгой безопасности (изоляции)** и **экономии контекстного окна (Context Budget Efficiency)**, MCP-серверы разделены на две фундаментальные категории:
...
│  (MCP Управления)            │                          │  (MCP Использования)         │
├──────────────────────────────┤                          ├──────────────────────────────┤
│ • Администрирование воркспейсов│                          │ • Выполнение запросов к базе │
│ • Управление API-ключами      │                          │ • Поиск в сети (SearXNG)     │
│ • Конфигурация поисковиков    │                          │ • Чтение/Запись файлов CRM   │
│ • Права и доступ пользователей│                          │ • Получение событий календаря│
└──────────────────────────────┘                          └──────────────────────────────┘
```

### 1. Control Plane MCP (Серверы Управления)
* **Назначение**: Административные инструменты для создания/удаления воркспейсов в AnythingLLM, настройки поисковых движков SearXNG и управления пользователями/правами в Nextcloud.
* **Зачем отделять**: 
  - **Безопасность (Изоляция)**: Исключает риск `Prompt Injection`, когда обычный пользователь Telegram мог бы случайно или намеренно заставить агента удалить воркспейс или переконфигурировать сервер.
  - **Экономия контекста**: Административные схемы инструментов не загружаются в контекст обычного пользовательского диалога.

### 2. Data Plane MCP (Серверы Использования)
* **Назначение**: Легковесные операционные инструменты для ежедневной работы (поиск по семантической базе, поиск в интернете, чтение/запись заметок и файлов пользователя).
* **Зачем отделять**: 
  - **Максимальная скорость и компактность**: В контекстное окно передаются только целевые функции вызова (Read/Write Data), что экономит тысячи токенов на каждом шаге рассуждения LLM.

---

## 🧠 Обоснование Троицы Инстансов
...
- **Model Context Protocol (MCP)**: Двухуровневая архитектура (Control Plane / Data Plane) для AnythingLLM, SearXNG и Nextcloud.
...
│   ├── mcp_config.py       # Менеджер конфигурации MCP серверов (Control & Data Planes)
